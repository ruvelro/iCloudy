...
# El campo de filtro de la app lleva su icono dentro, como cualquier campo de búsqueda del sistema.
FILTRO = '<svg viewBox="0 0 24 24"><path d="M4 7h16M7 12h10M10 17h4"/></svg>'
# Sin el aro exterior, ocho radios y un círculo pequeño se leen como un sol y no como un engranaje.
ENGR = ('<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="7.3"/><circle cx="12" cy="12" r="2.9"/>'
  '<path d="M12 2.7v2M12 19.3v2M21.3 12h-2M4.7 12h-2M18.6 5.4l-1.4 1.4M6.8 17.2l-1.4 1.4'
  'M18.6 18.6l-1.4-1.4M6.8 6.8 5.4 5.4"/></svg>')

# La barra de herramientas, en los cuatro grupos que separan sus reglas: Configuración | las acciones sobre los
# archivos | el modo de lista | las transferencias.
...
HERR = ''.join([
 ...
 '<svg viewBox="0 0 24 24"><path d="M7 4v16M4 17l3 3 3-3M17 20V4m-3 3 3-3 3 3"/></svg>',
 SEP,
 '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/>'
 '<path d="M9.6 8.4v7.2M7.5 13.5l2.1 2.1 2.1-2.1M14.4 15.6V8.4M12.3 10.5l2.1-2.1 2.1 2.1"/></svg>',
])
...
colecciones = "".join(
  '<span class="pestana{}">{}</span>'.format(" sel" if t == "Mis archivos" else "", t)
  for t in ["Mis archivos", "Recientes", "Compartido conmigo"])
...
explorador = f'''  <section class="principal">
    ...
    <div class="pestanas cols">{colecciones}</div>
    <div class="controles">
      <div class="miga"><span class="chip">Inicio</span><span>›</span><span class="chip">Trabajo</span></div>
      <div class="campo">{FILTRO}<span>Filtrar esta carpeta</span></div>
    </div>
    <div class="lista">
      <table><thead><tr><th>Nombre</th><th style="width:150px">Modificado</th><th style="width:110px">Tamaño</th></tr></thead>
      <tbody>{filas}</tbody></table>
    </div>
    <div class="pie"><span>{len(FILAS)} elementos</span><span>·</span><span class="fuerte">1 seleccionado</span>
      <span class="verdecito">✓ 3 en este Mac</span>
...
# El cajón va por pestañas, y cada una trae su propia escoba: lo que hay en curso, lo que acabó bien, lo que falló o
# se canceló, y el historial. "Completadas hoy" era una lista aparte y ahora es un filtro del historial.
PAUSA = '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M10 9v6M14 9v6"/></svg>'
PAPELERA = '<svg viewBox="0 0 24 24"><path d="M4 7h16M9.5 7V4.5h5V7M6.5 7l1 13h9l1-13"/></svg>'
pestanas_cajon = "".join(
  '<span class="pestana{}">{}</span>'.format(" sel" if t == "En curso" else "", t)
  for t in ["En curso", "Finalizadas", "Error", "Historial"])

cajon = f'''<aside class="cajon">
  <div class="cjcab"><span class="cjtit">Transferencias</span></div>
  <div class="pestanas cjt">{pestanas_cajon}</div>
  <div class="cjbar"><span>4 en curso</span><span class="der">{PAUSA}{PAPELERA}</span></div>
